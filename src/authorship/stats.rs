...
pub fn stats_for_commit_stats(
    ...
    let parent_commit_sha = if parent_count == 1 {
    ...
    let mut added_lines_by_file: HashMap<String, Vec<u32>> = if is_merge_commit {
        ...
        let from_ref = parent_commit_sha
            .clone()
    ...
    let (ai_accepted, ai_accepted_by_tool) = accepted_lines_from_attestations_with_duplicate_filter(
        authorship_log.as_ref(),
        &added_lines_by_file,
        is_merge_commit,
        Some(&mut duplicate_context),
    );
...
fn accepted_lines_from_attestations(
    ...
    accepted_lines_from_attestations_with_duplicate_filter(
        authorship_log,
        added_lines_by_file,
        is_merge_commit,
        None,
...
fn accepted_lines_from_attestations_with_duplicate_filter(
    authorship_log: Option<&crate::authorship::authorship_log_serialization::AuthorshipLog>,
    added_lines_by_file: &HashMap<String, Vec<u32>>,
    is_merge_commit: bool,
        ...
        for entry in &file_attestation.entries {
            ...
            for line in added_lines {
                if !entry
                    .line_ranges
                    .iter()
                    .any(|line_range| line_range.contains(*line))
                {
                    continue;
                }

                let duplicate_parent_ai_line =
                    duplicate_context.as_deref_mut().is_some_and(|context| {
                        context.is_duplicate_parent_ai_line(

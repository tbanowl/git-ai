...
fn test_diff_json_include_stats_exact_multi_model_with_non_landing_prompt() {
    ...
    //   - AI landed: 5 because mixed lines count on the AI side too
    //     (cursor-1, human-override, cursor-3, codex-a, codex-b2)
    ...
    let expected_breakdown = BTreeMap::from([
        ...
        ("cursor::gpt-4o".to_string(), tool_model_stats(3, 3, 0)),
    ]);
    assert_stats_exact(commit_stats, &expected_top_level, &expected_breakdown);
...
fn test_diff_json_include_stats_blame_deletions_devin_added_prompts_only() {
    ...
    assert!(
        commit_stats["tool_model_breakdown"]
            ...
            .expect("tool_model_breakdown should be an object")
            .is_empty(),
        "commit_stats is derived from authored notes; synthetic blame-only prompts should not contribute"
    );

    assert_eq!(commit_stats["ai_lines_added"], serde_json::json!(0));
    assert_eq!(commit_stats["git_lines_added"], serde_json::json!(2));
    assert_eq!(commit_stats["git_lines_deleted"], serde_json::json!(2));
    assert_eq!(commit_stats["human_lines_added"], serde_json::json!(2));
    assert_eq!(commit_stats["ai_lines_generated"], serde_json::json!(0));
}
